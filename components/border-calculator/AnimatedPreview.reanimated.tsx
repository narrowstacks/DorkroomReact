...
export const AnimatedPreview = React.memo(({ calculation, showBlades, borderColor }: AnimatedPreviewProps) => {
  ...
  const transformValues = useMemo(() => {
    ...
    const printCenterX = ((calculation.leftBorderPercent || 0) / 100) * containerWidth + printWidth / 2;
    const printCenterY = ((calculation.topBorderPercent || 0) / 100) * containerHeight + printHeight / 2;
  ...
  useEffect(() => {
    ...
    printTranslateX.value = withTiming(transformValues.printTranslateX, animationConfig);
    printTranslateY.value = withTiming(transformValues.printTranslateY, animationConfig);
    printScaleX.value = withTiming(transformValues.printScaleX, animationConfig);
    printScaleY.value = withTiming(transformValues.printScaleY, animationConfig);
    
    leftBladePosition.value = withTiming(transformValues.leftBorderPercent, animationConfig);
    rightBladePosition.value = withTiming(transformValues.rightBorderPercent, animationConfig);
    topBladePosition.value = withTiming(transformValues.topBorderPercent, animationConfig);
    bottomBladePosition.value = withTiming(transformValues.bottomBorderPercent, animationConfig, (finished) => {
      if (finished) {
        // Simple completion log without object sharing issues
        runOnJS(debugLog)('🎬 [REANIMATED] Animation cycle complete');
      }
    });
  }, [transformValues, printTranslateX, printTranslateY, printScaleX, printScaleY, leftBladePosition, rightBladePosition, topBladePosition, bottomBladePosition]);
  ...
  useAnimatedReaction(
    ...
    (currentValue, previousValue) => {
      ...
      if (previousValue !== null && Math.abs(currentValue - previousValue) > 0.1) {
        runOnJS(debugLog)('🎬 [REANIMATED WORKLET] Significant position change');
      }
    }
  );
  ...
  debugLog('✅ [REANIMATED PREVIEW] Rendering with dimensions:', staticDimensions);
