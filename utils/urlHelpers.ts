...
export const getNativeUrl = (encoded: string): string => {
  ...
  debugLog('🌐 [URL HELPER] Environment - DEV:', __DEV__, 'App ownership:', Constants.appOwnership);
  debugLog('🌐 [URL HELPER] Constants.expoConfig:', Constants.expoConfig);
  debugLog('🌐 [URL HELPER] window.location available:', typeof window !== 'undefined' && !!window.location);
  if (typeof window !== 'undefined' && window.location) {
  ...
  if (isDevelopment) {
    ...
    if (!hostUri && typeof window !== 'undefined' && window.location) {
      ...
      debugLog('🌐 [URL HELPER] Using window.location.host as hostUri:', hostUri);
    }
    ...
    if (hostUri) {
      ...
      debugLog('🌐 [URL HELPER] No hostUri available, falling back to production URL');
    }
  }
...
export const generateSharingUrls = (encoded: string) => {
  ...
  debugLog('🌐 [URL HELPER] Generated URLs - Web:', webUrl, 'Native:', nativeUrl);
