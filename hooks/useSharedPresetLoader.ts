...
export const useSharedPresetLoader = () => {
  const [loadedPreset, setLoadedPreset] = useState<{ name: string, settings: BorderPresetSettings, isFromUrl?: boolean } | null>(null);
  ...
  useEffect(() => {
    const handleUrl = (url: string | null) => {
      ...
      if (!url) {
        debugLog('🔗 [PRESET LOADER] No URL provided, clearing any existing preset');
        setLoadedPreset(null);
        return;
      }
      try {
        ...
        if (fragmentIndex !== -1) {
          ...
          if (parsedUrl.scheme === 'dorkroom' && parsedUrl.path?.startsWith('border')) {
            ...
            if (parsedUrl.queryParams?.preset) {
              ...
              debugLog('🔗 [PRESET LOADER] Found preset in query params:', encodedData);
            }
            // Fallback to old path-based format for backwards compatibility
              ...
              debugLog('🔗 [PRESET LOADER] Found preset in path (legacy):', encodedData);
            }
          }
          // Handle Expo Go development scheme
            ...
            debugLog('🔗 [PRESET LOADER] URL scheme not recognized for preset loading. Scheme:', parsedUrl.scheme, 'Path:', parsedUrl.path);
          }
        }

        if (encodedData) {
          debugLog('🔗 [PRESET LOADER] Attempting to decode preset data:', encodedData);
          const decodedPreset = decodePreset(encodedData);
          if (decodedPreset) {
            debugLog('🔗 [PRESET LOADER] Successfully decoded preset:', decodedPreset);
            debugLog('🔗 [PRESET LOADER] Setting loadedPreset state to:', decodedPreset);
